...
4. **UDP的接收端为什么可以接收很多发送端的消息？**

   接收端只负责接收数据包，无论是哪个发送端的数据包。

## TCP通信

**客户端开发（Client），使用java.net包下的Socket类来实现的。**

```java
public Socket(String host,int port) 根据指定的服务器ip，端口号请求与服务器建立连接，链接通过，就获得了客户端socket。
```
...
**服务端开发（Serve），使用java.net包下的ServerSocket类来实现的。**
...
```txt
//JDK8.0之前的方案 简单日期格式 SimpleDateFormat YYYY-MM-dd HH:mm:ss
SimpleDateFormat sdf = new SimpleDateFormat("YYYY年MM月dd日 HH:mm:ss EEE a");
System.out.println(sdf.format(System.currentTimeMillis()));

// JDK8.0的新方案 LocalDateTime LocalTime LocalDate
// 本地时间 时分秒
...
// 原理：StringBuilder底层维护了一个char[]数组，每次拼接都会将新字符串内容追加到数组末尾,是一个可变对象。
...
// 6.substring()：获取指定位置的字符串
sb1.substring(1,3);

// 7.toString()：将StringBuilder转换为String
sb1.toString();

// 8.setCharAt()：修改指定位置的字符
sb1.setCharAt(1,'w');

// 9.setLength()：修改字符串长度
sb1.setLength(5);

// 10.ensureCapacity()：修改StringBuilder的容量
...
// 做除法
BigDecimal b6 = b1.divide(b2, 2, BigDecimal.ROUND_HALF_UP);// 四舍五入
...
public class StringUtilTest {
    // 测试方法：无参数，无返回值，公开的，并且以@Test开头（junit测试框架识别）
    @Test
    public void testNameLength() {
        StringUtil util = new StringUtil();
        // 测试用例
        int length = util.getNameLength("张三");
        System.out.println(length);

        int length2 = util.getNameLength(" ");
        System.out.println(length2);

        int length3 = util.getNameLength("");
        System.out.println(length3);

        int length4 = util.getNameLength(null);
    ...
    public void testMaxIndex() {
        StringUtil util = new StringUtil();
        int index = util.getMaxIndex("abcdefg");
        System.out.println(index);

        int index1 = util.getMaxIndex("");
        System.out.println(index1);

        int index2 = util.getMaxIndex(null);
